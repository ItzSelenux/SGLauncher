	...
	if (event->type == GDK_BUTTON_PRESS && event->button == 3)
	{
		...
		if (path != NULL && model != NULL)
		{
			if (gtk_tree_model_get_iter(model, &iter, path))
			{
				...
				cmdexec = g_strdup_printf("%s -e %s", terminal, toexec);
				fmexec = g_strdup_printf("xdg-open %s", app_path);

				GtkWidget *menu = gtk_menu_new();
				GtkWidget *menu_item_run = gtk_menu_item_new_with_label("Run");
				GtkWidget *menu_item_runt = gtk_menu_item_new_with_label("Run in terminal");
				GtkWidget *menu_item_showfm = gtk_menu_item_new_with_label("Show entry in file manager");
				GtkWidget *menu_item_comment = gtk_menu_item_new_with_label(app_comment);
